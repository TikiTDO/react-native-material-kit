...
class Ripple extends Component {
  ...
  _onPointerMove(evt) {
    const { rippleX, rippleY } = this.state
    console.log("move")
    // Stop an active ripple if a touch has moved more than 10 units
    if (rippleX && rippleX) {
      const xDiff = rippleX - evt.x
      const yDiff = rippleY - evt.y
  ...
  stopRipple() {
    this.setState({ rippleX: undefined, rippleY: undefined })

    if (this._rippleAniStopper) {
      this._rippleAniStopper.stop()
      this._rippleAniStopper = undefined
    }

    this._animatedAlpha.setValue(0);
  ...
  showRipple() {
    ...
    this._rippleAni = Animated.sequence([
    ...
    this._rippleAni.start(() => {
      this._rippleAniStopper = this._rippleAni
